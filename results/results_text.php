				...
				<article>
					...
					<section>
						...
						<div class="uptop">
							...
							<form action="sample.php" method="POST">
								<table>
									...
									<tbody>
									<!-- results_text.php recieves the form submission from the search page using the text form. -->
									<?php 
									  // if html post is received than process the form
										if ($_SERVER["REQUEST_METHOD"] == "POST") {
											// check form parameters are set
											if( (isset($_POST['save_token'])) && (isset($_POST['text']))) {
												// check form parameters are non empty
												if ((!empty($_POST['save_token'])) && (!empty($_POST['text']))) {
													// check security token is unaltered
													if($_POST['save_token'] == 'abcdefg') {
							              // the search text is stored in a variable
														$text = $_POST['text'];
                            // create a php sql connection
							              ...
							              // if the connection is established
														if ($conne->connect_error) {
															...
															$search = "%{$text}%";
															$stmt = $conne->prepare("SELECT * FROM businesses WHERE name LIKE ?");
											        ...
											        // if the result is non empty; some businesses are returned from the query
															if ($output->num_rows > 0) {
																...
																$i = 1;
																// create a php array of the parameters required for the javascript mapping api
																$set = array();
						...
						<!-- this class is used for the section furthest to the bottom of the page -->
							...
							<?php //for ($set = array (); $row = $output->fetch_assoc(); $set[] = $row); ?>
							<?php //while( $result = $output->fetch_assoc()) { ?>
                    ...
                    crossorigin=""></script>
							...
							// if the form data is not set return invalid data
								echo "invalid_data";
						}
					}
				?>
